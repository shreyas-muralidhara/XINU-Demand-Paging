...
cirfifo_queue* frm_queue;
/*  added for the demand paging */
int page_replace_policy = SC;
int debug_flag = 0;

//Initialize the data structures required for demand paging
...
*/
	...
	/*Initialize backing store and page frame &*/
	init_bsm(); 
	init_frm(); 

	/* Initializing one one mapping for the first 4096 pages*/
	int pg_frm = 0;
	...
	for(i=0;i<4;i++){
		...
		frm_tab[pg_frm].fr_pid = NULLPROC;
		frm_tab[pg_frm].fr_type = FR_TBL;
		pt_t *pt_entry = (FRAME0 + pg_frm) * NBPG;
		...
		for(j=0;j<1024;j++){
			...
			pt_entry[j].pt_mbz = 0;
			pt_entry[j].pt_global = 1;
			pt_entry[j].pt_avail = 0;
			pt_entry[j].pt_base = (i * FRAME0) + j;

			//kprintf("pt_entry[%d].pt_base - %d, i = %d, j = %d, i*FRAME0= %d, (i*FRAME0)+j=%d\n",j,pt_entry[j].pt_base, i, j, i*FRAME0, (i*FRAME0)+j);
		}
		//kprintf("pt_entry[%d].pt_base = %d\n", i, pt_entry[0].pt_base);
	}
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	initialize_PD(NULLPROC);
	if(debug_flag)
		kprintf("NULLPROC tab pdbr = %d\n", proctab[NULLPROC].pdbr);
